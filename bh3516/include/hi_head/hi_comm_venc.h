...
{
    HI_U32 u32PicBytesNum;         /* the coded picture stream byte number for debug */
...
typedef struct hiVENC_ATTR_H264_S
{
	...
	HI_BOOL bVIField;			 /*the sign of the VI picture is field or frame. Invalidate for hi3516*/
}VENC_ATTR_H264_S;
...
typedef struct hiVENC_PARAM_H264E_ENTROPY_S
{
    ...
    HI_U32 cabac_stuff_en;                       /* Notes: just supported to be 0 for now*/
...
typedef struct hiVENC_PARAM_JPEG_S
{
    ...
    HI_U8  u32YQt[64];          /* y qt value */
    HI_U8  u32CbQt[64];         /* cb qt value */
    HI_U8  u32CrQt[64];         /* cr qt value */
...
typedef struct hiVENC_PARAM_MPEG4_S
{
    ...
    /* search window */
    HI_U32 u32HWSize;    /* size of horizontal search window.
                                * default value: differ with the picture sizes
                                * 0000: [-16, +15]
                                * 0001: [-32, +31]
                                */
    HI_U32 u32VWSize;    /* size of vertical search window.
...
typedef struct hiCROP_INOF_S
{
    ...
    HI_U32          u32AlignHeight;    /* aligned pic height */
    HI_U32          u32AlignWidth;     /* aligned pic width */
    HI_U32          u32PicHeight;      /* pic height */
    HI_U32          u32PicWidth;       /* pic width */
}CROP_INOF_S;

/* ROI struct */
typedef struct hiVENC_ROI_CFG_S
{
    HI_U32  u32Index;                    /* ROI region index number: 0~VENC_MAX_ROI_NUM-1 */
    HI_BOOL bEnable;                     /* ROI region enable */
    HI_BOOL bAbsQp;                      /* ROI QP enable */
    HI_S32  s32Qp;                       /* Qp value. When bAbsQp is HI_TRUE, the Qp value of the region is s32Qp.
                                            When bAbsQp is HI_FALSE, the Qp value of the region is the sum of the 
                                             picture QP and the s32Qp*/
    RECT_S  stRect;                      /* ROI region, note: s32X, s32Y, u32Width, u32Height must be multi of 16. */
}VENC_ROI_CFG_S;
...
typedef struct hiGROUP_VPP_CFG_S
{
    HI_S32    s32SrcFrmRate;             /* input frame rate */
    HI_S32    s32TarFrmRate;             /* output fram rate */
    GROUP_CLIP_CFG_S stClipCfg;          /* Clip configurate */
    VPP_CFG_S stVppCfg;                  /* hardware vpp configurate */
