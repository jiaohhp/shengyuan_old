...
dictionary * dictionary_new(int size)
{
    ...
    d->size = size ;	//每行存储的字节数

    d->val  = (char**)calloc(size, sizeof(char*));
    memset(d->val,0,size*sizeof(char*));	//指向行字符串的二维指针

    d->key  = (char**)calloc(size, sizeof(char*));
    memset(d->key, 0, size*sizeof(char*));//指向key字符串的二维指针

    d->hash = (unsigned int*)calloc(size, sizeof(unsigned));
    memset(d->hash, 0, size*sizeof(unsigned));//指向key字符串的指针

    /*2006/03/11 blair add : save comment and space Line*/
    if (size < DICTCOMMENTMINSZ) size = DICTCOMMENTMINSZ ;
    d->commSize = size ;	//一行注释的最大字节数

    d->comment  = (char**)calloc(size, sizeof(char*));
    memset(d->comment, 0, size*sizeof(char*));	//指向注释的二维指针

    d->commHash = (unsigned int*)calloc(size, sizeof(unsigned));
    memset(d->commHash, 0, size*sizeof(unsigned));//指向注释字符串指针

    d->commPlace= (unsigned char *)calloc(size, sizeof(unsigned char));
    memset(d->commPlace, 0, size*sizeof(unsigned char));//注释所在的位置
