...
void * memmap(unsigned int phy_addr, unsigned int size)
{
	...
	if(size == 0)
	{
		ptf_err("memmap():size can't be zero!\n");
		return NULL;
	}
	...
	if (addr == MAP_FAILED)
	{
		ptf_err("memmap():mmap @ 0x%x error!\n", phy_addr_in_page);
		return NULL;
	}
    ...
    if(NULL == pNew)
    {
        ptf_err("memmap():malloc new node failed!\n");
        return NULL;
    }
...
int memunmap(void * addr_mapped)
{
    ...
    if(pMMAPNode == NULL)
    {
        ptf_err("memunmap(): address have not been mmaped!\n");
        return -1;
    }
    ...
    do
	{
			...
			((unsigned int)addr_mapped <= (pTmp->Start_V + pTmp->length)) )
		{
            ...
            if(0 == pTmp->refcount)
            {
               ...
               // ptf_err("memunmap(): map node will be remove!\n");
                ...
                {
                    ptf_err("memunmap(): munmap failed!\n");
                }
...
void * mmapfile(char* fn, unsigned int size)
{
    ...
    if (pShm != NULL)
    {
        ptf_dbg("Memory Map OK. Map File: %s, length:%#X, addr:0x%08lX", 
                        fn, size, (unsigned long)pShm);
    }
    else
    {
        ptf_dbg("Memory Map Failure. Map File %s, length:0x%08lX", 
                        fn, size);
    }
    return pShm;        
}
